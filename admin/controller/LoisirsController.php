...
// EXECUTION DES FONCTIONS (qui sont dans EntityLoisirsRepository qu'ont rappel pour les fonctionnalité d'affichage  )
class LoisirsController
{
    private $db;
    public function __construct()
    {
        $this->db = new \model\EntityLoisirsRepository; // permet de récuperer une connexion a la BDD qui se trouve dans le fichier EntityLoisirsRepository.php
    }    
    ...
    // echo '<pre>';print_r($r); echo '</pre>';
        ...
        $this->render('layout.php', 'donnees.php', array(
            'title'=>'Loisirs', // $title  dans l'index
    ...
    public function save($op)
    {
        ...
        $values = ($op == 'update') ? $this->db->select($id) : ''; // si on a envoyé un id dans l'URL, on l'envoi en argument de la méthode select() de EntityLoisirsRepository, cela permettra de selectionner toute les données de l'élément pour les modification.
        ...
        if($_POST)
        {
            $r = $this->db->save(); // lorque l'on valide le formulaire d'ajout, on execute la methode save() du fichier EntityLoisirsRepository .
            $this->redirect('loisirs.php');
        }
