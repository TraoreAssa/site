...
class Controller
{
    ...
    # permet de savoir ce que l'internaute demande (afficher/modifier/supprimer), action de l'internaute 
    {
        ...
        try {
            ...
            # si op est definie dans l'URL, on le recupere, on le stock dans $op sinon, si rien n'est definie dans l'URL,on stock NULL dans $op
            if ($op == 'add' || $op == 'update') $this->save($op);
            # si on ajoute ou modifie un élément, on appel la méthode save();
            elseif ($op == 'select') $this->select();
            # si on selectionne un élément, on appel la méthode select();
            elseif ($op == 'delete') $this->delete();
            # si on supprime un élément, on appel la méthode delete();
            # elseif($op == 'cv')$this->cv(); 
            else $this->selectAll();
            # permettra d'afficher l'ensemble des donnees;  
    ...
    {
        extract($parameters);
        # permet d'avoir mes paramettres (des tableau en) dans une variable
        ob_start();
        # commence la temporisation, ob_start()demarrer la temporisation de sortie
        ...
        # tout ce qui se trouve dans $template sera stocké dans $content
        # $content = "view/$template"; 

        ob_start();
        # temporiser la sortie de l'affichage
        require "view/$layout";
        return ob_end_flush();
        # permet de liberer l'affichage et fait tout apparaitre sur la page
    }

    #------------------------------------------ REDIRIGE VERS  ------------------------------------------ 
    public function redirect($url)
    # methode permettant d'effectuer un redirection
    {
        header("Location:" . $url);
        # fonction prédefinie permettant d'effectuer une redirection
    }
    ...
    public function save($op)
    {
        ...
        $id = isset($_GET['id']) ? $_GET['id'] : 'NULL';
        # permet de savoir si un id a été envoyé dans l'URL, si on clique sur 'modifier' on envoi l'id dans l'URL et on le recupere, sinon c'est un ajout

        $values = ($op == 'update') ? $this->db->select($id) : '';
        # si on a envoyé un id dans l'URL, on l'envoi en argument de la méthode select() de EntityRepository, cela permettra de selectionner toute les données de l'élément pour les modification.



        if ($_POST) {
            $r = $this->db->save();
            # lorque l'on valide le formulaire d'ajout, on execute la methode save() du fichier EntityRepository .
