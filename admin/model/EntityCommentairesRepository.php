...
class EntityCommentairesRepository
{
    private $db;
    public $table;
    public function getDb() // méthode permettant d'instancier la classe PDO et de créer un objet
    {
        if (!$this->db) // seulement si $this->db n'est pas rempli, il n'y a pas de connexion alors on la construit
        {
            ...
            try {
                ...
                $this->table = $xml->table9; // on associe le nom de la table du fichier XML a la propriété de la classe
                try 
...
$a = new EntityCommentairesRepository; 
$a->getDb();
