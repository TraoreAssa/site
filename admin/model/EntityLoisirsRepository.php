    ...
    public function getDb() // méthode permettant d'instancier la classe PDO et de créer un objet
    {
        if (!$this->db) // seulement si $this->db n'est pas rempli, il n'y a pas de connexion alors on la construit
        {
            ...
            try {
                ...
                $this->table = $xml->table4; // on associe le nom de la table du fichier XML a la propriété de la classe
                try 
...
/************ AFFICHER UN ELEMENT ************/
    ...
    {
        $q = $this->getDb()->query("SELECT * FROM " . $this->table . " WHERE id"."_" . $this->table . "=" . (int) $id);
...
/************ SUPPRIMER ************/
    ...
    {
        $q = $this->getDb()->query("DELETE FROM " . $this->table . " WHERE id"."_" . $this->table. '=' . (int) $id);
    }
}

$a = new EntityLoisirsRepository; 
$a->getDb();
