    ...
    public function getDb() // méthode permettant d'instancier la classe PDO et de créer un objet
    {
        if (!$this->db) // seulement si $this->db n'est pas rempli, il n'y a pas de connexion alors on la construit
        {
            ...
            try {
                ...
                $this->table = $xml->table8; // on associe le nom de la table du fichier XML a la propriété de la classe
                $this->tableUtilisateur = $xml->table;
                try 
...
/************ AJOUTER ET MODIFIER (REMPLACE si id est n'est pas nul) ************/
    public function save()
    {
        ...
        // var_dump($q);
        // $resultat = $bdd()->query('REPLACE INTO ' . 'photo' . '(id'. '_' . 'photo' . ',' .  implode(',', array_keys($_POST)). ') VALUES (' . $id . ',' . "'"  . implode("','", $_POST) . "'" . ')');
        $q = $this->getDB()->query('REPLACE INTO ' . 'photo' . '(id'. '_' . 'photo' . ',' .  implode(',', array_keys($_POST)). ') VALUES (' . $id . ',' . "'"  . implode("','", $_POST) . "'" . ')');
